...
run_tests()
{
    # Makes pipes return a fail status if the first command failed.
    set -o pipefail
...
create_documentation()
{
    ...
    # Make docs.
    cd "$WORKSPACE"/doc
    make latexpdf POLAR2GRID_DOC="$prefix" || status=1
    # Copy pdfs to package directory.
    cp "$WORKSPACE"/doc/build/latex/*.pdf "${WORKSPACE}/$package_name" || status=1
    ...
    make html POLAR2GRID_DOC="$prefix" || status=1
    cd "$WORKSPACE"
...
for prefix in ${prefixes}; do
    # Allows one package to be successful and published even if the other fails.
    package_status=0
